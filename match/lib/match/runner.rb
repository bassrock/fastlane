module Match
  class Runner
    ...
    def run(params)
      ...
      if spaceship
        app_identifiers.each do |app_identifier|
          spaceship.bundle_identifier_exists(username: params[:username], app_identifier: app_identifier, params: params[:platform])
        end
      end
    ...
    def fetch_provisioning_profile(params: nil, certificate_id: nil, app_identifier: nil)
      ...
      profile_name = [Match::Generator.profile_type_name(prov_type), app_identifier, platform].join("_").gsub("*", '\*') # this is important, as it shouldn't be a wildcard
